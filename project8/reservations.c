...
int seat_count;
int broker_count;
int *seat_taken;  // Array of seats (this is a critical section)
int transaction_count;

int seat_taken_count = 0; // (this is a critical section)
...
int reserve_seat(int n)
{
    ...
    // mark seat as taken
    seat_taken[n] = 1;
    seat_taken_count++; // increment the total count
    pthread_mutex_unlock(&mutex);
...
int free_seat(int n)
{
    ...
    seat_taken[n] = 0;
    seat_taken_count--; // decrement the total count
    pthread_mutex_unlock(&mutex);
...
int verify_seat_count(void) {
    ...
    int count_result = (count == seat_taken_count);
